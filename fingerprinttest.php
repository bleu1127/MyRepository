            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <div class="alert alert-info">
                        For testing purposes only. Input a fingerprint ID to simulate attendance.
                    </div>
                    <form id="simulation-form" class="mb-3">
                        <div class="input-group">
                            <input type="number" 
                                   class="form-control" 
                                   id="simulated-fingerprint-id" 
                                   placeholder="Enter Fingerprint ID">
                            <button class="btn btn-primary" type="submit">
                                <i class="fas fa-fingerprint me-2"></i>Simulate Scan
                            </button>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('simulation-form').addEventListener('submit', async function(e) {
        ...
        if (!fingerprintId) {
            updateStatus('Please enter a fingerprint ID', 'warning');
            return;
        }
        
        updateStatus('Processing simulated fingerprint...', 'info');
        document.getElementById('fingerprintIcon').style.color = '#007bff';
        await processAttendance(fingerprintId);
    });
    ...
    async function processAttendance(fingerprintId, confirmLogout = false) {
        try {
            ...
            if (result.confirmation_required) {
                ...
                const simulationInput = document.getElementById('simulated-fingerprint-id');
    ...
    setInterval(updateStudentProfile, 2000);
    ...
    setInterval(refreshAttendanceData, 2000);
});
</script>
